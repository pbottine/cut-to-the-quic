...
public:
  ...
  uint32_t hash() const
  {
    ...
    /// get current hash
  /** @return 32 bit XXHash **/
  uint32_t hash_no_final_bit_mixing() const
  {
    uint32_t result = (uint32_t)totalLength;
  ...
  // ========== Modification by Paul Bottinelli ==========
  // Added for differential cryptanalysis attack demonstration
  // Modified hash computation that processes 4-byte blocks without final mixing
  // This allows analysis of intermediate hash states for differential cryptanalysis
  uint32_t hash_single_round() const
  {
    uint32_t result = (uint32_t)totalLength;
    ...
    // result = rotateLeft(result + *(uint32_t*)data * Prime3, 17) * Prime4;
    result = result + *(uint32_t*)data * Prime3;
    result = rotateLeft(result, 17);
    result = result * Prime4;
  ...
  // ========== Modification by Paul Bottinelli ==========
  // Static wrapper for single round hash computation
  static uint32_t hash_single_round(const void* input, uint64_t length, uint32_t seed)
  {
    XXHash32 hasher(seed);
    hasher.add(input, length);
    return hasher.hash_single_round();
  }

  static uint32_t hash_no_final_bit_mixing(const void* input, uint64_t length, uint32_t seed)
  {
    XXHash32 hasher(seed);
    hasher.add(input, length);
    return hasher.hash_no_final_bit_mixing();
  }
  // ========== End Modification ==========
